...
disp(char(strcat('ARX:', {' '}, datestr(datetime))))
na_range = 1:4;
na_size = ones(2,2);
...
disp(char(strcat('OE:', {' '}, datestr(datetime))))
nb_range = 1:4;
nb_size = ones(2,2);
nb = bsxfun(@times,nb_size,reshape(nb_range,1,1,numel(nb_range)));
...
for nb_i=1:size(nb_range,2)
    for nf_i=1:size(nf_range,2)
        for nk_i=1:size(nk_range,2)
            if (nb_i > nf_i), break, end
            i = i+1;
            
            models_oe(i).name = strcat('nb_',int2str(nb_i), ...
                                       '_nf_',int2str(nf_i), ...
                                       '_nk_',int2str(nk_i));
                                   
            models_oe(i).model = oe(train, [nb(:,:,nb_i) ...
                                            nf(:,:,nf_i) ...
                                            nk(:,:,nk_i)]);
