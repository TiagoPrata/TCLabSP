...
estab_time = 1500;          % Tempo para estabilização do sistema

train_valid_rate = 0.6;     % Relacao entre quant. de treinamento e validacao (ex. 60% treinamento -> 40% validacao)
...
%% Criando modelo
tclabsp = iddata([ensaios.('Sensor1'), ensaios.('Sensor2')],[ensaios.('Aquecedor1'), ensaios.('Aquecedor2')],21);
...
data_length = length(find(ensaios.('Time') >= estab_time));         % Quant de amostras, excluindo estabilizacao
train_start = find(ensaios.('Time') >= estab_time, 1, 'first');     % Encontra primeiro indice apos estabilizacao
...
%sys = tfest(train, [2 2; 2 2], [1 1; 1 1],'Ts',Ts_target);
                    ...
                    for zS1H2 = 1:max_zeros
                        ...
                        for zS2H1 = 1:max_zeros
                            ...
                            for zS2H2 = 1:max_zeros
                                ...
                                i = i+1;
                                models_tf(i).name = strcat('tf', int2str(pS1H1), int2str(pS1H2), int2str(pS2H1), int2str(pS2H2), int2str(zS1H1), int2str(zS1H2), int2str(zS2H1), int2str(zS2H2));
                                models_tf(i).model = tfest(train, [pS1H1 pS1H2; pS2H1 pS2H2], [zS1H1 zS1H2; zS2H1 zS2H2],'Ts',Ts_target);
                                [y,fit,x0] = compare(valid, models_tf(i).model);
                                models_tf(i).param_num = pS1H1+pS1H2+pS2H1+pS2H2+zS1H1+zS1H2+zS2H1+zS2H2;
                                models_tf(i).fit = fit;
                            end
                        end
                    end
...
for i=1:10
    ...
    models_ss(i).name = ssest(train, i, 'Ts',Ts_target);
...
for na_i=1:size(na_range,2)
    for nb_i=1:size(nb_range,2)
        for nk_i=1:size(nk_range,2)
            i = i+1;
            models_arx(i).name = strcat('na_',int2str(na_i),'_nb_',int2str(nb_i),'_nk_',int2str(nk_i));
            models_arx(i).model = arx(train, [na(:,:,na_i) nb(:,:,nb_i) nk(:,:,nk_i)]);
            models_arx(i).aic = aic(models_arx(i).model);
            [y,fit,x0] = compare(valid, models_arx(i).model);
            models_arx(i).fit = fit;
            
            models_oe(i).name = strcat('nb_',int2str(na_i),'_nf_',int2str(nb_i),'_nk_',int2str(nk_i));
            models_oe(i).model = oe(train, [na(:,:,na_i) nb(:,:,nb_i) nk(:,:,nk_i)]);
...
for na_i=1:size(na_range,2)
    for nb_i=1:size(nb_range,2)
        for nc_i=1:size(nc_range,2)
            for nk_i=1:size(nk_range,2)
                i = i+1;
                models_armax(i).name = strcat('na_',int2str(na_i),'_nb_',int2str(nb_i),'_nc_',int2str(nc_i),'_nk_',int2str(nk_i));
                models_armax(i).model = armax(train, [na(:,:,na_i) nb(:,:,nb_i) nc(:,:,nc_i) nk(:,:,nk_i)]);
...
%% Cria modelos bj
%sys = bj(train, [2*ones(2,2), 2*ones(2,1), 2*ones(2,1), 2*ones(2,2), ones(2,2)]);
    ...
    for nc_i=1:size(nc_range,2)
        for nd_i=1:size(nd_range,2)
            for nf_i=1:size(nf_range,2)
                for nk_i=1:size(nk_range,2)
                    i = i+1;
                    models_bj(i).name = strcat('nb_',int2str(nb_i),'_nc_',int2str(nc_i),'_nd_',int2str(nd_i),'_nf_',int2str(nf_i),'_nk_',int2str(nk_i));
                    models_bj(i).model = bj(train, [nb(:,:,nb_i) nc(:,:,nc_i) nd(:,:,nd_i) nf(:,:,nf_i) nk(:,:,nk_i)]);
...
% na = [S1-TermsOutputS1 S1-TermsOutputS2; S2-TermsOutputS1 S2-TermsOutputS2]
% nb = [S1-NumTermosH1 S1-NumTermosH2; S2-NumTermosH1 S2-NumTermosH2]
                            ...
                            for nk_2=0:2
                                for nk_3=0:2
                                    for nk_4=0:2
                                        ...
                                        Orders = [na,nb,nk];
                                        models_nlarx(i).name = strcat('na',int2str(na_1),'00',int2str(na_4),'_nb',int2str(nb_1),int2str(nb_2),int2str(nb_3),int2str(nb_4),'_nk',int2str(nk_1),int2str(nk_2),int2str(nk_3),int2str(nk_4));
